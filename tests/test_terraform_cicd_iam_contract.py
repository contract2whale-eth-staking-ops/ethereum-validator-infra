...
class TerraformCicdIamContractTests(unittest.TestCase):
    ...
    def test_bootstrap_outputs_map_only_into_the_tracked_runtime_manifest(self) -> None:
        readme = BOOTSTRAP_README.read_text(encoding="utf-8")
        self.assertIn("NodePermissionsBoundaryArn", readme)
        self.assertIn("NodeSshKeyPairName", readme)
        self.assertIn("primary-aws/terraform/ci/runtime-inputs.json", readme)
        self.assertIn(".aws.node_permissions_boundary_arn", readme)
        self.assertIn("KmsBreakGlassRoleArn", readme)
        self.assertIn(".aws.kms_break_glass_role_arn", readme)
        self.assertIn(".terraform.key_pair_name", readme)

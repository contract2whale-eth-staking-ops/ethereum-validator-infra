...
class CodeownersRenderTests(unittest.TestCase):
    ...
    def test_renders_exact_organization_teams_and_is_idempotent(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            first = self.run_helper(target, "dev-playbuilder47-eth-staking-ops/ethereum-validator-infra")
            self.assertEqual(first.returncode, 0, first.stderr)
            self.assertIn(
                "CODEOWNERS_RENDER=OK org=dev-playbuilder47-eth-staking-ops changed=true", first.stdout
            )
            rendered = target.read_text(encoding="utf-8")
            self.assertEqual(rendered.count("@dev-playbuilder47-eth-staking-ops/platform-approvers"), 5)
            self.assertEqual(rendered.count("@dev-playbuilder47-eth-staking-ops/security-approvers"), 7)
            self.assertNotIn("YOUR_GITHUB_ORG", rendered)

            second = self.run_helper(target, "dev-playbuilder47-eth-staking-ops/ethereum-validator-infra")
            self.assertEqual(second.returncode, 0, second.stderr)
            self.assertIn(
                "CODEOWNERS_RENDER=OK org=dev-playbuilder47-eth-staking-ops changed=false", second.stdout
            )
    ...
    def test_rejects_invalid_repository_foreign_owner_and_symlink(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = self.run_helper(foreign, "dev-playbuilder47-eth-staking-ops/repository")
            self.assertNotEqual(result.returncode, 0)
            self.assertIn("reason=unexpected_content", result.stderr)

            link = root / "link"
            link.symlink_to(invalid_repo)
            result = self.run_helper(link, "dev-playbuilder47-eth-staking-ops/repository")
            self.assertNotEqual(result.returncode, 0)

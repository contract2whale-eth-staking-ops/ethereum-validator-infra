variable "admin_cidrs" {
  description = "SSH 허용 CIDR 목록. IPv4 host route(/32)만 허용한다."
  type        = list(string)
...
variable "backup_peer_public_ip" {
  description = "Standby EC2의 public IPv4. null이면 WireGuard ingress를 만들지 않는다."
  type        = string
  default     = null

  validation {
    condition = var.backup_peer_public_ip == null || (
      can(regex("^([0-9]{1,3}\\.){3}[0-9]{1,3}$", var.backup_peer_public_ip)) && try(
        cidrhost("${var.backup_peer_public_ip}/32", 0) == var.backup_peer_public_ip,
        false,
      )
    )
    error_message = "backup_peer_public_ip must be null or one canonical IPv4 address."
  }
}
